...
def handle_registration_response(
    ...
    try:
        ...
        if is_success and result_cd == "000":
            ...
            if not item_code:
                frappe.log_error(
                    "[SMART] Missing itemCd in payload/request_data/response",
            ...
            if not item_name:
                frappe.log_error(
                    f"[SMART] Item with code {item_code} not found",
                    "Registration Handler"
                )
                return
            ...
            zra_item_code = (
                result_data.get("itemCd")
                or result.get("ItemCd")
                or item_code
            )
        ...
        frappe.log_error(
            ...
            f"[SMART] Failed to process registration response: {e}"
        )
...
def fetch_matching_items_on_success(response: dict, document_name: str, settings_name: str, **kwargs) -> None:
    ...
    # --- CASE 2: No remote item found, trigger new item creation ---
        ...
        if "no search result" in error_message or "error code: 001" in error_message:
            frappe.logger().info(f"[SMART] No existing ZRA item found for {item_doc.name}, creating new item.")
...
def item_archive_on_success(response: dict = None, **kwargs):
