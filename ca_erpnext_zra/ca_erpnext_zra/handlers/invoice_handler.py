...
def check_and_credit_invoice_revision(
    ...
    frappe.logger("zra_sync").info(
        f"Checking invoice revision for {document_name} (Ref: {reference_number})"
    )
    ...
    def on_success(response, **_):
        if response.get("IsSuccess"):
            frappe.logger("zra_sync").info(
                f"Successfully credited revision {reference_number} in ZRA."
            )
